...
class Head(nn.Module):
    """
    MLP network predicting per-pixel scene coordinates and uncertainty.
    ...
    def __init__(self,
        ...
        # --- MODIFICATION START ---
        # Modify the final layer to output an extra channel for uncertainty
        num_output_channels = 3 # Default for (x, y, z)
        if self.use_homogeneous:
            num_output_channels = 4 # For homogeneous coords (x, y, z, w)

        # Add 1 channel for the uncertainty (log variance)
        self.fc3 = nn.Conv2d(self.head_channels, num_output_channels + 1, 1, 1, 0)
        # --- MODIFICATION END ---
    ...
    def forward(self, res):
        ...
        sc_and_uncertainty = F.relu(self.fc1(res))
        sc_and_uncertainty = F.relu(self.fc2(sc_and_uncertainty))
        sc_and_uncertainty = self.fc3(sc_and_uncertainty)

        # --- MODIFICATION START ---
        # Separate the output into scene coordinates and uncertainty
        if self.use_homogeneous:
            sc = sc_and_uncertainty[:, :4]
            log_variance = sc_and_uncertainty[:, 4, :, :].unsqueeze(1)
            # Dehomogenize coords:
            ...
            sc = sc_and_uncertainty[:, :3]
            log_variance = sc_and_uncertainty[:, 3, :, :].unsqueeze(1)

        # Add the mean to the predicted coordinates.
        sc += self.mean
        
        # Return both scene coordinates and the log variance
        return sc, log_variance
...
class Regressor(nn.Module):
    ...
    def create_from_state_dict(cls, state_dict):
        ...
        use_homogeneous = state_dict["heads.fc3.weight"].shape[0] == 5
