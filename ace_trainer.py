...
class TrainerACE:
    ...
    def create_training_buffer(self):
        ...
        with torch.no_grad():
            ...
            buffer_idx = 0
            dataset_passes = 0
            training_buffer_size = self.options.training_buffer_size
            ...
            while buffer_idx < self.options.training_buffer_size:
                ...
                for image_B1HW, image_mask_B1HW, gt_pose_B44, gt_pose_inv_B44, intrinsics_B33, intrinsics_inv_B33, _, _ in training_dataloader:
                    ...
                    ptsanddesc = self.regressor.ptsanddesc(pts_tensor[:,:2],desc_tensor)
                    #print(ptsanddesc.dtype)
                    #print(features_NC.dtype)
                    features_NC_crossAttention = self.regressor.trans_encoder(features_NC,ptsanddesc)
                    #print(features_NC.shape)

                    batch_data = {
                        'features': features_NC_crossAttention,
    ...
    def save_model(self):
        ...
        model_state_dict["ptsanddesc"] = {k: v.half() for k, v in self.regressor.ptsanddesc.state_dict().items()}
        model_state_dict["trans_encoder"] = {k: v.half() for k, v in self.regressor.trans_encoder.state_dict().items()}
        ...
        _logger.info(f"Saved trained weights for superpoint, ptsanddesc, attention, and heads to: {self.options.output_map_file}")