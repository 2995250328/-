    ...
    def create_training_buffer(self):
        ...
        with torch.no_grad():
            ...
            while buffer_idx < self.options.onebuffer:
                dataset_passes += 1
                for image_RGB_raw, image_RGB_B3HW , image_B1HW, image_mask_B1HW, gt_pose_B44, gt_pose_inv_B44, intrinsics_B33, intrinsics_inv_B33, _, _ in training_dataloader:

                    # Copy to device.
                    image_RGB_B3HW = image_RGB_B3HW.to(self.device, non_blocking=True)
                    ...
                    with autocast('cuda', enabled=self.options.use_half):
                        features = self.regressor.get_features(image_RGB_B3HW)

                    # 4(1*64*173*231) 8(1*128*87*116) 16(1*256*44*58) 32(1*640*22*29)
                    (features_BCHW_4, features_BCHW_8, features_BCHW_16, features_BCHW_32, features_BCHW_cls) = features

                    # Dimensions after the network's downsampling.
                    B, C, H, W = features_BCHW_4.shape 
                    ...
                    batch_data = {
                        'features': normalize_shape(features_BCHW_4),
