...
from ace_network_pe import Regressor
...
class TrainerACE:
    def __init__(self, options):
        ...
        self.repro_loss = ReproLoss(
            total_iterations=self.iterations * self.options.training_buffer_size // self.options.onebuffer,
    ...
    def create_training_buffer(self):
        ...
        # X = torch.arange(3000,device=self.device, dtype=torch.float16)
        # Y = X.reshape(-1,1)
        # # 初始化位置编码
        # pos_enc = torch.zeros((1,1, 3000, 3000), device=self.device, dtype=torch.float16)
        # for i in range(0, 3000, 2):
        #     pos_enc[:,:, i, :] += torch.sin(X * (10000 ** (2 * i / 3000)))
        #     pos_enc[:,:, i+1, :] += torch.cos(X * (10000 ** (2 * i / 3000)))
        #     pos_enc[:,:, :, i] += torch.sin(Y * (10000 ** (2 * i / 3000))).squeeze(1)
        #     pos_enc[:,:, :, i+1] += torch.cos(Y * (10000 ** (2 * i / 3000))).squeeze(1)
        ...
        self.training_buffer = {
            'features': torch.empty((self.options.onebuffer, self.regressor.feature_dim*2),
        ...
        with torch.no_grad():
            ...
            while buffer_idx < self.options.onebuffer:
                ...
                for _,image_B1HW, image_mask_B1HW, gt_pose_B44, gt_pose_inv_B44, intrinsics_B33, intrinsics_inv_B33, _, _ in training_dataloader:
                    ...
                    # image_B1HW = self.regressor.se(image_B1HW)
                    # image_B1HW = image_B1HW + pos_enc[:,:,:image_B1HW.shape[2],:image_B1HW.shape[3]]
                    ...
                    with autocast(enabled=self.options.use_half):
                        ...
                        with torch.enable_grad():
                            pyramid_BCHW = self.regressor.se(features_BCHW)

                    features_BCHW = torch.cat([features_BCHW,pyramid_BCHW],dim=1)
                    ...
                    # features_BCHW = torch.cat((features_BCHW, X_enc, Y_enc), dim=1)
                    ...
                    # with autocast(enabled=self.options.use_half):
                    #     features_BCHW = self.regressor.se(features_BCHW)
    ...
    def training_step(self, features_bC, target_px_b2, gt_inv_poses_b34, Ks_b33, invKs_b33):
        ...
        features_bC[:,512:] += torch.empty_like(features_bC[:,512:]).normal_(
                                                            mean=0,std=0.1,generator=self.gn_generator)
        features_bC[:,512:] = torch.nn.functional.normalize(features_bC[:,512:],dim=1)
    ...
    def save_model(self):
        ...
        model_state_dict["se"] = {k: v.half() for k, v in self.regressor.se.state_dict().items()}
        model_state_dict["pyramidpooling"] = {k: v.half() for k, v in self.regressor.pyramidpooling.state_dict().items()}
        model_state_dict["heads"] = {k: v.half() for k, v in self.regressor.heads.state_dict().items()}

        # Save the dictionary to a file
        torch.save(model_state_dict, self.options.output_map_file)

        _logger.info(f"Saved trained weights for pyramidpooling, and heads to: {self.options.output_map_file}")