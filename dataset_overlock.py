...
class CamLocDataset(Dataset):
    ...
    def _get_single_item(self, idx, image_height):
        ...
        if self.augment:
            ...
            image_RGB = self._rotate_image(image_RGB, angle, 1, 'reflect')
        ...
        image_RGB_raw = denormalize(image_RGB.clone(), [0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        image_RGB_raw = torch.clamp(image_RGB_raw * 255.0, 0, 255).byte()
        ...
        return image_RGB_raw, image_RGB, image, image_mask, pose, pose_inv, intrinsics, intrinsics_inv, coords, str(self.rgb_files[idx])
