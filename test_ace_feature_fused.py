...
from ace_network_feature_fused import Regressor
...
if __name__ == '__main__':
    ...
    superpoint = SuperPointFrontend(opt.weights_path,opt.nms_dist,opt.conf_thresh,opt.nn_thresh,cuda)
    ...
    with torch.no_grad():
        for image_RGB,image_B1HW, _, gt_pose_B44, _, intrinsics_B33, _, _, filenames in testset_loader:
            ...
            with autocast(enabled=True):
                scene_coordinates_BN3,patch_idx,fH,fW = network(image_B1HW,pts,desc)
            
    

            # Each frame is processed independently.
            for frame_idx, (scene_coordinates_N3, gt_pose_44, intrinsics_33, frame_path) in enumerate(
                    zip(scene_coordinates_BN3, gt_pose_B44, intrinsics_B33, filenames)):
                ...
                print(scene_coordinates_N3.shape)
                print(points_2d.shape)
                inlier_count = dsacstarsample.forward_sequence_rgb(
                    scene_coordinates_N3.float().cpu().contiguous(),
                    points_2d,
                    inliers,
                    out_pose,
                    4,
