...
if __name__ == '__main__':
    ...
    filename = str(head_network_path).split('/')[-1]
    ...
    X = torch.arange(3000,device=device, dtype=torch.float16)
    Y = X.reshape(-1,1)
    # 初始化位置编码
    pos_enc = torch.zeros((1,1, 3000, 3000), device=device, dtype=torch.float16)
